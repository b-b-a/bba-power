 ...
 * @author     Shaun Freeman <shaun@shaunfreeman.co.uk>
 */
abstract class Power_Model_Abstract
{
    /**
    ...
    public function getDbTable($name)
	{
        if (!isset($this->_dbTables[$name])) {
            $class = join('_', array(
                ...
                'Model',
                'DbTable',
                BBA_Utility_String::getInflected($name)
            ));
     ...
     */
    public function setCache(Power_Model_Cache $cache)
    {
        $this->_cache = $cache;
    }
    ...
    public function getCached($tagged = null)
    {
        if (null === $this->_cache) {
            ...
            if ($this->_cacheOptions['useCache']) {
                $this->_cache = new Power_Model_Cache(
                    $this,
                    $options
                );
            }
